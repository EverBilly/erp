                  ...
                  <Form>
                    <Grid container spacing={3}>
                      <Grid item xs={12} sm={6}>
                        <Field name="username">
                          {({ field }) => (
                            <TextField {...field} label="Nombre de usuario" fullWidth
                              error={touched.username && !!errors.username}
                              helperText={touched.username && errors.username}
                              disabled={isEdit} variant="outlined" size="small" sx={{ mb: 2 }}
                            />
                          )}
                        </Field>
                      </Grid>
                            ...
                            <FormControlLabel
                              control={
                                <Switch checked={field.value}
                                  onChange={(e) => setFieldValue('active', e.target.checked)} color="primary"
                                />
                              }
                      ...
                      {!isEdit ? (
                        <Grid item xs={12}>
                          <Field name="password">
                            {({ field }) => (
                              <TextField {...field} label="Contraseña"
                        ...
                        <Grid item xs={12}>
                          <Field name="password">
                            {({ field }) => (
                              <TextField {...field} label="Nueva Contraseña (opcional)"
                                type={showPassword ? "text" : "password"} fullWidth
                                error={touched.password && !!errors.password}
                        ...
                        <FormControlLabel
                          control={
                            <Switch checked={showPassword}
                              onChange={(e) => setShowPassword(e.target.checked)} color="primary"
                            />
                          }
                      ...
                      <Grid item xs={12}>
                        <Autocomplete
                          ...
                          value={availableRoles.filter(role => validRoleIds.includes(role.id))}
                          onChange={(event, newValue) => {
                            setFieldValue('roleIds', newValue.map(role => role.id));
                          }}
                          renderInput={(params) => (
                            <TextField {...params} label="Asignar Roles"
                      ...
                      <Grid item xs={12}>
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 2 }}>
                          <Button onClick={() => navigate('/users')} variant="outlined" size="large">
                            Cancelar
                          </Button>
                          ...
                          >
                            {isSubmitting ? (
                              <><CircularProgress size={20} sx={{ mr: 1 }} />Guardando...</>
